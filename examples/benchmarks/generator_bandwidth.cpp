...
std::pair<double,double> run_benchmark(size_t matrix_size)
{
    ...
    for (unsigned int i = 0; i < matrix_size; ++i)
        for (unsigned int j = 0; j < matrix_size; ++j)
            stl_A[i*matrix_size + j] = random<ScalarType>();

    for (unsigned int i = 0; i < matrix_size; ++i)
        for (unsigned int j = 0; j < matrix_size; ++j)
            stl_B[i + j*matrix_size] = random<ScalarType>();

    viennacl::matrix<ScalarType> vcl_A(matrix_size, matrix_size);
    viennacl::matrix<ScalarType,FB> vcl_B(matrix_size, matrix_size);
    viennacl::matrix<ScalarType,FC> vcl_U(matrix_size, matrix_size);
    viennacl::matrix<ScalarType,FC> vcl_V(matrix_size, matrix_size);
...

    {
        ...
        op.add(dma_t(vcl_U) = dmb_t(vcl_A) + dmc_t(vcl_B));
        op.add(dma_t(vcl_V) = dmb_t(vcl_A) - dmc_t(vcl_B));
        op.execute();
    ...
    {
        viennacl::generator::custom_operation op_add;
        op_add.add(dma_t(vcl_U) = dmb_t(vcl_A) + dmc_t(vcl_B));
        viennacl::generator::custom_operation op_sub;
        op_sub.add(dma_t(vcl_U) = dmb_t(vcl_A) - dmc_t(vcl_B));
        op_add.execute();
        viennacl::backend::finish();
        op_sub.execute();
        viennacl::backend::finish();
    ...
    for(unsigned int k=0 ; k < num_platforms ; ++k)
    {
        ...
        for(devices_type::iterator it = dev.begin() ; it != dev.end() ; ++it){
            ...
            if(current_device++ == requested_device ){
                ...
                std::cout << "#Size \t 2 Kernels : Execution Time \t 1 Kernel : Execution Time" << std::endl;
                for(unsigned int size = SIZE_INC ; size <= MAX_SIZE ; size += SIZE_INC){
                    unsigned int n_read = 4;
                    unsigned int n_write = 2;
                    std::pair<double,double> exec_time = run_benchmark<NumericT,viennacl::row_major,viennacl::row_major>(size);
                    std::cout << size << "\t" << exec_time.second
                                      << "\t" << exec_time.first << std::endl;
                }
            }
        }
    }

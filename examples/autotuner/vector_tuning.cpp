...
void fill_timings(TimingsT & timings, OpT const & op_template){
    ...
    unsigned int max_local_size = dev.max_workgroup_size();

    unsigned int min_local_size;
    ...
    if (dev.type() == CL_DEVICE_TYPE_GPU)
    {
        ...
        min_local_size = 32; //less than 32 threads per work group is unlikely to have any impact
    ...
    for(unsigned int u = min_unroll ; u <= max_unroll ; u*=2){
        for(unsigned int a = min_alignment ; a <= max_alignment ; a*=2){
            for(unsigned int wg = min_work_groups ; wg <= max_work_groups; wg*=2){
...
std::pair<double, viennacl::generator::code_generation::blas1_optimization_profile> autotune(){
...
int main(){
	...
	for(unsigned int k=0 ; k < num_platforms ; ++k)
	{
		...
		for(devices_type::iterator it = dev.begin() ; it != dev.end() ; ++it){
            ...
            std::pair<double, viennacl::generator::code_generation::blas1_optimization_profile> best = autotune();

      ...
      private:
        ...
        struct CHECK_OPERATIONS_STRUCTURE
        {
          ...
          public:
            ...
            static const unsigned int n_products = tree_utils::count_if<Products,is_pure_product_leaf>::value ;
            static const int n_nested_products = n_products - typelist_utils::length<Products>::value;

            static void execute()
            {
                VIENNACL_STATIC_ASSERT(is_inplace_product == false,InplaceProductsForbidden);
                VIENNACL_STATIC_ASSERT(n_products ==1,OnlyOneProductLine);
